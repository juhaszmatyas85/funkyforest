	...
	class WPCleverWoosb {
                ...
                <div class="<?php echo esc_attr( $settings_class ); ?>">
					<?php if ( $active_tab === 'how' ) { ?>
                        <div class="wpclever_settings_page_content_text">
                            ...
                            <p>
                                <img src="<?php echo esc_url( WOOSB_URI . 'assets/images/how-01.jpg' ); ?>"/>
                            </p>
                        </div>
                            ...
                            <table class="form-table">
                                ...
                                <tr>
                                    ...
                                    <td>
										...
										esc_attr_e( 'Choose %s', 'woo-product-bundle' ); ?>"/>
		...
		function cart_item_name( $name, $cart_item ) {
			if ( ! empty( $cart_item['woosb_parent_id'] ) && ( WPCleverWoosb_Helper()->get_setting( 'hide_bundle_name', 'no' ) === 'no' ) ) {
				...
				return get_the_title( $parent_id ) . ' &rarr; ' . esc_html( $name );
			}
                ...
                <table>
                    ...
                    <tr class="woosb_tr_space">
                        ...
                        <td>
                            <div class="woosb-actions">
                                ...
                                </a> <select id="woosb_bulk_actions">
                    ...
                    <tr class="woosb_tr_space woosb_tr_show_if_auto_price">
                        ...
                        <td style="vertical-align: middle; line-height: 30px;">
                            ...
                            <input id="woosb_discount_amount" name="woosb_discount_amount" type="number" min="0" step="0.0001" style="width: 80px" value="<?php echo esc_attr( get_post_meta( $product_id, 'woosb_discount_amount', true ) ); ?>"/> <?php echo esc_html( get_woocommerce_currency_symbol() ); ?>
                    ...
                    <tr class="woosb_tr_space">
                        ...
                        <td>
                            ...
                            <span class="woosb_show_if_total_limits">
                                <?php esc_html_e( 'Min', 'woo-product-bundle' ); ?> <input id="woosb_total_limits_min" name="woosb_total_limits_min" type="number" min="0" style="width: 80px" value="<?php echo esc_attr( get_post_meta( $product_id, 'woosb_total_limits_min', true ) ); ?>"/>
                                <?php esc_html_e( 'Max', 'woo-product-bundle' ); ?> <input id="woosb_total_limits_max" name="woosb_total_limits_max" type="number" min="0" style="width: 80px" value="<?php echo esc_attr( get_post_meta( $product_id, 'woosb_total_limits_max', true ) ); ?>"/> <?php echo esc_html( get_woocommerce_currency_symbol() ); ?>
                            </span>
                        </td>
                    </tr>
                    <tr class="woosb_tr_space">
                    ...
                    <tr class="woosb_tr_space">
                        ...
                        <td>
                            <input type="text" name="woosb_custom_price" value="<?php echo esc_attr( get_post_meta( $product_id, 'woosb_custom_price', true ) ); ?>"/> E.g:
                            <code>From $10 to $100</code>. <?php /* translators: dynamic price */
		...
		function product_data_li( $product, $item, $key = '', $search = false ) {
			...
			$optional      = isset( $item['optional'] ) ? $item['optional'] : 0;
				...
				if ( ! $product->is_fixed_price() ) {
					...
					if ( $product->has_optional() ) {
						if ( $price_format === 'min_only' || $price_format === 'from_min' ) {
							...
							// min whole
							$min_whole = (float) ( get_post_meta( $product_id, 'woosb_limit_whole_min', true ) ?: 1 );
							$min_price = min( array_column( $items_optional, 'price' ) );
							...
							switch ( $price_format ) {
								case 'min_only':
									$price = apply_filters( 'woosb_get_price_html_min_only', wc_price( $total_min ) . $product->get_price_suffix(), $total_min, $product );
									break;
								case 'from_min':
									$price = apply_filters( 'woosb_get_price_html_from_min', '<span>' . esc_html__( 'From', 'woo-product-bundle' ) . '</span> ' . wc_price( $total_min ) . $product->get_price_suffix(), $total_min, $product );
									break;
							}
						}
						...
						foreach ( $items as $item ) {
							...
							if ( isset( $data[1] ) ) {
								if ( is_numeric( $data[1] ) ) {
									...
									$key = $data[1];
									$qty = (float) ( isset( $data[2] ) ? $data[2] : 1 );
								}
							} else {
								$key = WPCleverWoosb_Helper()->generate_key();
								$qty = 1;
